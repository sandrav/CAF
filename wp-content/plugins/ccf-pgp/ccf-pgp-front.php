...
if (!class_exists('ccfpgpFront')) {
	class ccfpgpFront extends ccfpgp {
		...
		function insertFrontEndStyles() {
            wp_register_style('CCFStandardsCSS', plugins_url() . '/ccf-pgp/css/ccf-pgp-standards.css');
           	wp_register_style('CCFFormsCSS', plugins_url() . '/ccf-pgp/css/ccf-pgp.css');
		...
		function insertFrontEndScripts() { 
			...
			wp_enqueue_script('ccf-main', plugins_url() . '/ccf-pgp/js/ccf-pgp.js', '1.0');
			wp_enqueue_script('ccf-custom', plugins_url() . '/ccf-pgp/js/custom.js', '1.0');
		}
		
		function insertDatePickerScripts() {
			wp_enqueue_script('jquery-ui-datepicker', plugins_url() . '/ccf-pgp/js/jquery.ui.datepicker.js', array('jquery-ui-core', 'jquery-ui-widget'));
			wp_enqueue_script('ccf-datepicker', plugins_url() . '/ccf-pgp/js/ccf-pgp-datepicker.js', '1.2');
		}
		...
		function contentFilter($content) {
			...
			if (!empty($errors)) {
				$admin_options = parent::getAdminOptions();
				$out = '<div id="ccf-pgp-errors"><p>'.$admin_options['default_form_error_header'].'</p><ul>' . "\n";
				//$errors = $this->getAllFormErrors();
				foreach ($errors as $error) {
					$out .= '<li>'.$error.'</li>' . "\n";
				}
				$err_link = (!empty($this->error_return)) ? '<p><a href="'.$this->error_return.'" title="'.__('Go Back', 'ccf-pgp').'">&lt; ' . __('Go Back to Form.', 'ccf-pgp') . '</a></p>' : '';
		...
		function getFormCode($form, $is_widget_form = false) {
			$startFormItem = '<div class="formItem">' . "\n";
			$endFormItem = "\n" . '</div>' . "\n";
			...
			$out = '';
			$form_styles = '';
			$style_class = (!$is_widget_form) ? ' ccfpgp' : ' ccfpgp-sidebar';
			...
			$form_title = ccfpgp_utils::decodeOption($form->form_title, 1, 1);
			...
			//$out .=  '<script type="text/javascript">'.$form->behavior.'</script>';
			$out .= ccfpgp_utils::decodeOption($form->custom_code, 1, 1) . "\n";
			if (!empty($form_title) && !$is_widget_form) $out .= '<h2 id="h2-' . $form->id . '-' . $form_key . '" class="formTitle">' . $form_title . '</h2>' . "\n";
			$fields = parent::getAttachedFieldsArray($form->id);
			$hiddens = '';
			$code_type = ($admin_options['code_type'] == 'XHTML') ? ' /' : '';
			$add_reset = '';
			$out .='<div class="formContent">';
			foreach ($fields as $field_id) {
				$field = parent::selectField($field_id, '');
				$req = ($field->field_required == 1 or $field->field_slug == 'ishuman') ? '* ' : '';
				$req_long = ($field->field_required == 1) ? ' ' . __('(required)', 'ccf-pgp') : '';
				$input_id = 'id="'.ccfpgp_utils::decodeOption($field->field_slug, 1, 1).'-'.$form_key.'"';
				$field_value = ccfpgp_utils::decodeOption($field->field_value, 1, 1);
				...
				//pgpchk
				$input_privatename = ccfpgp_utils::decodeOption(($field->field_encrypted==1)? $field->field_slug."_".CCF_PRIVATE : $field->field_slug , 1, 1);
				...
				if (isset($_SESSION['ccf_fields'][$field->field_slug])) {
					...
					$out .= $startFormItem . $this->getCaptchaCode($field, $form->id) . "\n" . '</div>' . "\n";
				} elseif ($field->field_slug == 'usaStates') {
					$field->field_value = $field_value;
					$out .= $startFormItem . $this->getStatesCode($field, $form->id) . "\n" . '</div>' . "\n";
				} elseif ($field->field_slug == 'ishuman') {
					$field->field_value = $field_value;
					$out .= $startFormItem . $this->getIsHumanCode($field, $form->id) . "\n" . '</div>' . "\n";
				} elseif ($field->field_slug == 'allCountries') {
					$field->field_value = $field_value;
					$out .= $startFormItem . $this->getCountriesCode($field, $form->id) . "\n" . '</div>' . "\n";
				...
				//pgpchk
				// change names
				//$field->field_slug x $field->field_slug."_pgpchk_private"
				
				} elseif ($field->field_type == 'Text') {
					$maxlength = (empty($field->field_maxlength) or $field->field_maxlength <= 0) ? '' : ' maxlength="'.$field->field_maxlength.'"';
					//pgpchk $out .= '<div>'."\n".'<label for="'.ccfpgp_utils::decodeOption($field->field_slug, 1, 1).'">'. $req .ccfpgp_utils::decodeOption($field->field_label, 1, 1).'</label>'."\n".'<input class="'.$field->field_class.' '.$tooltip_class.'" '.$instructions.' '.$input_id.' type="text" name="'.ccfpgp_utils::decodeOption($field->field_slug, 1, 1).'" value="'.$field_value.'"'.$maxlength.''.$code_type.'>'."\n".'</div>' . "\n";
					$out .= $startFormItem.'<label for="'.ccfpgp_utils::decodeOption($field->field_slug, 1, 1).'">'. $req .ccfpgp_utils::decodeOption($field->field_label, 1, 1).'</label>'."\n".'<input class="'.$field->field_class.' '.$tooltip_class.'" '.$instructions.' '.$input_id.' type="text" name="'.$input_privatename.'" value="'.$field_value.'"'.$maxlength.''.$code_type.'>'."\n".'</div>' . "\n";
					//$input_privatename
				} elseif ($field->field_type == 'File') {
					$file_upload_form = ' enctype="multipart/form-data" ';
					//$out .= '<div>'."\n".'<label for="'.ccfpgp_utils::decodeOption($field->field_slug, 1, 1).'">'. $req .ccfpgp_utils::decodeOption($field->field_label, 1, 1).'</label>'."\n".'<input class="'.$field->field_class.' '.$tooltip_class.'" '.$instructions.' '.$input_id.' type="file" name="'.ccfpgp_utils::decodeOption($field->field_slug, 1, 1).'" value="'.$field_value.'"'.$code_type.'>'."\n".'</div>' . "\n";
					$out .= $startFormItem.'<label for="'.ccfpgp_utils::decodeOption($field->field_slug, 1, 1).'">'. $req .ccfpgp_utils::decodeOption($field->field_label, 1, 1).'</label>'."\n".'<input class="'.$field->field_class.' '.$tooltip_class.'" '.$instructions.' '.$input_id.' type="file" name="'.$input_privatename.'" value="'.$field_value.'"'.$code_type.'>'."\n".'</div>' . "\n";
				} elseif ($field->field_type == 'Date') {
					$maxlength = (empty($field->field_maxlength) or $field->field_maxlength <= 0) ? '' : ' maxlength="'.$field->field_maxlength.'"';
					//$out .= '<div>'."\n".'<label for="'.ccfpgp_utils::decodeOption($field->field_slug, 1, 1).'">'. $req .ccfpgp_utils::decodeOption($field->field_label, 1, 1).'</label>'."\n".'<input class="'.$field->field_class.' ccf-datepicker '.$tooltip_class.'" '.$instructions.' '.$input_id.' type="text" name="'.ccfpgp_utils::decodeOption($field->field_slug, 1, 1).'" value="'.$field_value.'"'.$maxlength.''.$code_type.'>'."\n".'</div>' . "\n";
					$out .= $startFormItem.'<label for="'.ccfpgp_utils::decodeOption($field->field_slug, 1, 1).'">'. $req .ccfpgp_utils::decodeOption($field->field_label, 1, 1).'</label>'."\n".'<input class="'.$field->field_class.' ccf-datepicker '.$tooltip_class.'" '.$instructions.' '.$input_id.' type="text" name="'.$input_privatename.'" value="'.$field_value.'"'.$maxlength.''.$code_type.'>'."\n".'</div>' . "\n";
				} elseif ($field->field_type == 'Hidden') {
					//$hiddens .= '<input type="hidden" name="'.ccfpgp_utils::decodeOption($field->field_slug, 1, 1).'" value="'.$field_value.'" '.$input_id.''.$code_type.'>' . "\n";
					$hiddens .= '<input type="hidden" name="'.ccfpgp_utils::decodeOption($field->field_slug, 1, 1).'" value="'.$field_value.'" '.$input_id.''.$code_type.'>' . "\n";
				} elseif ($field->field_type == 'Checkbox') {
					//$out .= '<div>'."\n".'<input class="'.$field->field_class.' '.$tooltip_class.'" '.$instructions.' type="checkbox" name="'.ccfpgp_utils::decodeOption($field->field_slug, 1, 1).'" value="'.ccfpgp_utils::decodeOption($field->field_value, 1, 1).'" '.$input_id.''.$code_type.'> '."\n".'<label class="checkbox" for="'.ccfpgp_utils::decodeOption($field->field_slug, 1, 1).'">' . $req .ccfpgp_utils::decodeOption($field->field_label, 1, 1).'</label>'."\n".'</div>' . "\n";
					$out .= $startFormItem.'<input class="'.$field->field_class.' '.$tooltip_class.'" '.$instructions.' type="checkbox" name="'.$input_privatename.'" value="'.ccfpgp_utils::decodeOption($field->field_value, 1, 1).'" '.$input_id.''.$code_type.'> '."\n".'<label class="checkbox" for="'.ccfpgp_utils::decodeOption($field->field_slug, 1, 1).'">' . $req .ccfpgp_utils::decodeOption($field->field_label, 1, 1).'</label>'."\n".'</div>' . "\n";
				} elseif ($field->field_type == 'Textarea') {
					//$out .= '<div>'."\n".'<label for="'.ccfpgp_utils::decodeOption($field->field_slug, 1, 1).'">'. $req .ccfpgp_utils::decodeOption($field->field_label, 1, 1).'</label>'."\n".'<textarea class="'.$field->field_class.' '.$tooltip_class.'" '.$instructions.' '.$input_id.' rows="5" cols="40" name="'.ccfpgp_utils::decodeOption($field->field_slug, 1, 1).'">'.$field_value.'</textarea>'."\n".'</div>' . "\n";
					$out .= $startFormItem.'<label for="'.ccfpgp_utils::decodeOption($field->field_slug, 1, 1).'">'. $req .ccfpgp_utils::decodeOption($field->field_label, 1, 1).'</label>'."\n".'<textarea class="'.$field->field_class.' '.$tooltip_class.'" '.$instructions.' '.$input_id.' rows="5" cols="40" name="'.$input_privatename.'">'.$field_value.'</textarea>'."\n".'</div>' . "\n";
					...
					if (!empty($options)) {
						//if (!$is_widget_form) $out .= '<div>'."\n".'<label for="'.ccfpgp_utils::decodeOption($field->field_slug, 1, 1).'">'. $req .ccfpgp_utils::decodeOption($field->field_label, 1, 1).'</label>'."\n".'<select '.$instructions.' '.$input_id.' name="'.ccfpgp_utils::decodeOption($field->field_slug, 1, 1).'" class="'.$field->field_class.' '.$tooltip_class.'">'."\n".$field_options.'</select>'."\n".'</div>' . "\n";
						if (!$is_widget_form) $out .= $startFormItem.'<label for="'.ccfpgp_utils::decodeOption($field->field_slug, 1, 1).'">'. $req .ccfpgp_utils::decodeOption($field->field_label, 1, 1).'</label>'."\n".'<select '.$instructions.' '.$input_id.' name="'.$input_privatename.'" class="'.$field->field_class.' '.$tooltip_class.'">'."\n".$field_options.'</select>'."\n".'</div>' . "\n";
						//else  $out .= '<div>'."\n".'<label for="'.ccfpgp_utils::decodeOption($field->field_slug, 1, 1).'">'. $req .ccfpgp_utils::decodeOption($field->field_label, 1, 1).'</label>'."\n".'<select class="'.$field->field_class.' '.$tooltip_class.'" '.$instructions.' '.$input_id.' name="'.ccfpgp_utils::decodeOption($field->field_slug, 1, 1).'">'."\n".$field_options.'</select>'."\n".'</div>' . "\n";
						else  $out .= $startFormItem.'<label for="'.ccfpgp_utils::decodeOption($field->field_slug, 1, 1).'">'. $req .ccfpgp_utils::decodeOption($field->field_label, 1, 1).'</label>'."\n".'<select class="'.$field->field_class.' '.$tooltip_class.'" '.$instructions.' '.$input_id.' name="'.$input_privatename.'">'."\n".$field_options.'</select>'."\n".'</div>' . "\n";
					}
					...
					foreach ($options as $option_id) {
						...
						//$field_options .= '<div><input'.$option_sel.' class="'.$field->field_class.' '.$tooltip_class.'" type="radio" '.$instructions.' name="'.ccfpgp_utils::decodeOption($field->field_slug, 1, 1).'" value="'.ccfpgp_utils::decodeOption($option->option_value, 1, 1).'"'.$code_type.'> <label class="select" for="'.ccfpgp_utils::decodeOption($field->field_slug, 1, 1).'">' . ccfpgp_utils::decodeOption($option->option_label, 1, 1) . '</label></div>' . "\n";
						$field_options .= $startFormItem.'<input'.$option_sel.' class="'.$field->field_class.' '.$tooltip_class.'" type="radio" '.$instructions.' name="'.$input_privatename.'" value="'.ccfpgp_utils::decodeOption($option->option_value, 1, 1).'"'.$code_type.'> <label class="select" for="'.ccfpgp_utils::decodeOption($field->field_slug, 1, 1).'">' . ccfpgp_utils::decodeOption($option->option_label, 1, 1) . '</label>' . $endFormItem;
					}
					$field_label = (!empty($field->field_label)) ? '<label for="'.ccfpgp_utils::decodeOption($field->field_slug, 1, 1).'">'. $req .ccfpgp_utils::decodeOption($field->field_label, 1, 1).'</label>' : '';
					if (!empty($options)) $out .= '<div class="formGroup">'."\n".$field_label."\n".$field_options."\n".'</div>' . "\n";
				} elseif ($field->field_type == 'Checkbox') {
					$field_options = '';
					$options = parent::getAttachedFieldOptionsArray($field->id);
					$z = 0;
					foreach ($options as $option_id) {
						...
						$check_value = (empty($option->option_value)) ? $option->option_label : ccfpgp_utils::decodeOption($option->option_value, 1, 1);
						//$field_options .= '<div><input'.$option_sel.' class="'.$field->field_class.' '.$tooltip_class.'" type="checkbox" '.$instructions.' name="'.ccfpgp_utils::decodeOption($field->field_slug, 1, 1).'['.$z.']" value="'.$check_value.'"'.$code_type.'> <label class="select" for="'.ccfpgp_utils::decodeOption($field->field_slug, 1, 1).'">' . ccfpgp_utils::decodeOption($option->option_label, 1, 1) . '</label></div>' . "\n";
						$field_options .= '<div><input'.$option_sel.' class="'.$field->field_class.' '.$tooltip_class.'" type="checkbox" '.$instructions.' name="'.$input_privatename.'['.$z.']" value="'.$check_value.'"'.$code_type.'> <label class="select" for="'.ccfpgp_utils::decodeOption($field->field_slug, 1, 1).'">' . ccfpgp_utils::decodeOption($option->option_label, 1, 1) . '</label></div>' . "\n";
						$z++;
					}
					$field_label = (!empty($field->field_label)) ? '<label for="'.ccfpgp_utils::decodeOption($field->field_slug, 1, 1).'">'. $req .ccfpgp_utils::decodeOption($field->field_label, 1, 1).'</label>' : '';
			...
			if (!empty($file_upload_form))
				...
				$submit_text = (!empty($form->submit_button_text)) ? ccfpgp_utils::decodeOption($form->submit_button_text, 1, 0) : 'Submit';
			
			$out .= '<input name="form_page" value="'.$_SERVER['REQUEST_URI'].'" type="hidden"'.$code_type.'>'."\n".'<input name="form_page_name" value="'.get_queried_object()->post_name.'" type="hidden"'.$code_type.'>'."\n".'<input type="hidden" name="fid" value="'.$form->id.'"'.$code_type.'>'."\n".$hiddens."\n";
			$out .= '<input type="submit" id="submit-' . $form->id . '-'.$form_key.'" class="formSubmit" value="' . $submit_text . '" name="ccfpgp_submit"'.$code_type.'>';
			...
			if ($form->form_style != 0) {
				...
				$form_styles .= '#' . $form_id . " { width: ".$style->form_width."; text-align:left; padding:".$style->form_padding."; margin:".$style->form_argin."; border:".$style->form_borderwidth." ".$style->form_borderstyle." #".parent::formatStyle($style->form_bordercolor)."; background-color:#".parent::formatStyle($style->form_backgroundcolor)."; font-family:".$style->form_fontfamily."; } \n";
		...
		function processFileUpload($field) {
			$errors = array();
			if (empty($_FILES[$field->field_slug])) $errors[] = __('An error occured while uploading: ', 'ccf-pgp') . $field->field_slug;
			$admin_options = parent::getAdminOptions();
			if ($field->field_max_upload_size > 0 && $_FILES[$field->field_slug]['size'] > ($field->field_max_upload_size * 1000)) $errors[] = basename($_FILES[$field->field_slug]['name']) . __(' is too large of a file. The maximum file size for that field is ', 'ccf-pgp') . $field->field_max_upload_size . __(' KB.', 'ccf-pgp');
			...
			if (!empty($allowed_exts))
				if (!in_array($ext, $allowed_exts)) $errors[] = '.' . $ext . __(' is an invalid file extension.', 'ccf-pgp');
			if (!empty($errors)) return $errors;
			
			// create necessary directories
			if (!is_dir(ABSPATH."wp-content/plugins/ccf-pgp/uploads/".date("Y")))
				mkdir(ABSPATH."wp-content/plugins/ccf-pgp/uploads/".date("Y"));
			if (!is_dir(ABSPATH . "wp-content/plugins/ccf-pgp/uploads/".date("Y")."/".date("m")))
				mkdir(ABSPATH . "wp-content/plugins/ccf-pgp/uploads/".date("Y")."/".date("m"));
			
			// check if file already exists
			$file_name = preg_replace('/(.*)\..*/i', '$1', basename($_FILES[$field->field_slug]['name']));
			$file_name_addon = ".";
			$i = 1;
			while (file_exists( ABSPATH . "wp-content/plugins/ccf-pgp/uploads/".date("Y")."/".date("m")."/" . $file_name . $file_name_addon . $ext)) {
				$file_name_addon = ' ('.$i.').';
				$i++;
			}
			$target_path = ABSPATH . "wp-content/plugins/ccf-pgp/uploads/".date("Y")."/".date("m")."/" . $file_name . $file_name_addon . $ext;
			$this->form_uploads[$field->field_slug] = ABSPATH . "wp-content/plugins/ccf-pgp/uploads/".date("Y")."/".date("m")."/" . $file_name . $file_name_addon . $ext;
			if(!move_uploaded_file($_FILES[$field->field_slug]['tmp_name'], $target_path)) {
				// Error!
				$errors[] = __('An error occured while uploading: ', 'ccf-pgp') . $field->field_slug;
			}
			return $errors;
		}
		
		function processForms() {
			if (isset($_POST['ccf_customhtml']) || isset($_POST['ccfpgp_submit'])) {
				// BEGIN define common language vars
				$lang = array();
				$lang['field_blank'] = __('You left this field blank: ', 'ccf-pgp');
				$lang['form_page'] = __('Form Displayed on Page: ', 'ccf-pgp');
				$lang['sender_ip'] = __('Sender IP: ', 'ccf-pgp');
				...
				$admin_options = parent::getAdminOptions();
				$fixed_customhtml_fields = array('required_fields', 'success_message', 'thank_you_page', 'destination_email', 'ccf_customhtml', 'form_page_name');
				$req_fields = $this->requiredFieldsArrayFromList($_POST['required_fields']);
				$req_fields = array_map('trim', $req_fields);
				$body = '';
				$form_page_name = $_POST['form_page_name'];
				//echo $_POST['form_page_name'];
				...
				if (empty($errors)) {
					...
					//$data_object = new ccfpgpUserData(array('data_array' => $data_array, 'form_page' => $_SERVER['SERVER_NAME']. $_SERVER['REQUEST_URI'], 'form_id' => 0, 'data_time' => time()));
					$data_object = new ccfpgpUserData(array('data_array' => $data_array, 'form_page_name' => $form_page_name, 'form_id' => 0, 'data_time' => time()));
					...
					if ($admin_options['email_form_submissions'] == 1) {
						...
						ccfpgp_utils::redirect($_POST['thank_you_page']);
					}
				...
				unset($_POST);
				//pgpchk
				//forms build by widget
			} elseif (isset($_POST['ccfpgp_submit'])) {
				ccfpgp_utils::startSession();
					 ...
					 if ($field->field_slug == 'ishuman') {
						if (!isset($_POST['ishuman']) || (isset($_POST['ishuman']) && $_POST['ishuman'] != 1)) {
							if (empty($field->field_error))
								$this->setFormError('ishuman', __('Only humans can use this form.', 'ccf-pgp'));
							else $this->setFormError('ishuman', $field->field_error);
						}
					} elseif ($field->field_slug == 'captcha') {
						if ($_POST['captcha'] != $_SESSION[$cap_name]) {
							if (empty($field->field_error))
								$this->setFormError('captcha', __('You copied the number from the captcha field incorrectly.', 'ccf-pgp'));
						...
						if (!$this->validEmail($_POST['fixedEmail'])) {
							if (empty($field->field_error))
								$this->setFormError('fixedEmail', __('The email address you provided is not valid.', 'ccf-pgp'));
						...
						if (!$this->validWebsite($_POST['fixedWebsite'])) {
							if (empty($field->field_error))
								$this->setFormError('fixedWebsite', __('The website address you provided is not valid.', 'ccf-pgp'));
						...
						//cambia nombre pgpchk
						$post_slug = ($field->field_encrypted == 1)? $field->field_slug."_".CCF_PRIVATE : $field->field_slug; 
						if ($field->field_required == 1 && $field->field_type != 'File' && !empty($_POST[$post_slug])) {
				...
				foreach ($_POST as $key => $value) {
					...
					if (!array_key_exists($key, $GLOBALS['ccf_fixed_fields']) || $key == 'fixedEmail' || $key == 'usaStates' || $key == 'fixedWebsite'|| $key == 'emailSubject' || $key == 'allCountries') {
						...
						//pgpchk 
						//echo $key." - ".$value;
						$isPrivate=strpos($key,CCF_PRIVATE);
						if(!$isPrivate===FALSE) {
							$pkey=substr($key, 0, $isPrivate-1);
							$private_array[$pkey]=$value;
						}else{
							$data_array[$key] = $value;
						}
					...
					$file_url = preg_replace('/^.*(\/ccf-pgp\/.*)$/i', plugins_url() . '$1', $upload);
					...
					$lang['not_checked'] = __('Not Checked', 'ccf-pgp');
				...
				if (empty($errors)) {
					...
					//$data_object = new ccfpgpUserData(array('private_array' => $private_array, 'data_array' => $data_array,  'form_page' => $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'], 'form_id' => $form->id, 'data_time' => $post_time));
					$data_object = new ccfpgpUserData(array('private_array' => $private_array, 'data_array' => $data_array,  'form_page_name' => $form_page_name, 'form_id' => $form->id, 'data_time' => $post_time));
                    //pgpchk
                    //echo "pasado a ccfpgpUserData object en forms_front<br />";
                    //print_r(array('private_array' => $private_array, 'data_array' => $data_array,  'form_page' => $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'], 'form_id' => $form->id, 'data_time' => $post_time));
					...
					if ($admin_options['email_form_submissions'] == '1') {
						...
						$from_name = (empty($admin_options['default_from_name'])) ? __('CCF PGP', 'ccf-pgp') : $admin_options['default_from_name'];
						...
						$mail->AltBody = __("To view the message, please use an HTML compatible email viewer.", 'ccf-pgp');
						$mail->CharSet = 'utf-8';
						$mail->MsgHTML(stripslashes($body));
						$mail->Send();
					} if (!empty($form->form_thank_you_page)) {
						ccfpgp_utils::redirect($form->form_thank_you_page);
					}
		...
		function getCaptchaCode($field_object, $form_id) {
			...
			$out = '<img width="96" height="24" alt="' . __('Captcha image for CCF PGP plugin. You must type the numbers shown in the image', 'ccf-pgp') . '" id="captcha-image" src="' . get_bloginfo('wpurl') . '/wp-content/plugins/ccf-pgp/image.php?fid='.$form_id.'"'.$code_type.'> 
		...
		function getStatesCode($field_object, $form_id) {
			ccfpgp_utils::load_module('extra_fields/states_field.php');
			...
			return "\n".'<label for="'.ccfpgp_utils::decodeOption($field_object->field_slug, 1, 1).'">'. $req .ccfpgp_utils::decodeOption($field_object->field_label, 1, 1).'</label>'.$states_field->getCode();
		}
		
		function getDatePickerCode($field_object, $form_id, $xhtml_code) {
			ccfpgp_utils::load_module('extra_fields/date_field.php');
			$req = ($field_object->field_required == 1) ? '* ' : '';
			...
			return "\n".'<label for="'.ccfpgp_utils::decodeOption($field_object->field_slug, 1, 1).'">'. $req .ccfpgp_utils::decodeOption($field_object->field_label, 1, 1).'</label>'.$date_field->getCode();
		}
		
		function getCountriesCode($field_object, $form_id) {
			ccfpgp_utils::load_module('extra_fields/countries_field.php');
			$req = ($field_object->field_required == 1) ? '* ' : '';
			...
			return '<label for="'.ccfpgp_utils::decodeOption($field_object->field_slug, 1, 1).'">'. $req .ccfpgp_utils::decodeOption($field_object->field_label, 1, 1).'</label>' . "\n" . $countries_field->getCode();
		}
