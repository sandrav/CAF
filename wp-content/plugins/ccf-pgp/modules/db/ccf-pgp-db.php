...
if (!class_exists('ccfpgpDB')) {
	class ccfpgpDB {
		...
		function insertForm($form) {
			...
			$skip_encode = array('form_access','behavior');
			$save_code = array('behavior');
			foreach ($form as $key => $value)
				if (!in_array($key, $skip_encode))
					$form[$key] = ccfpgp_utils::encodeOption($value);
				if (in_array($key, $save_code))
		...
		function insertField($field, $fixed = false, $skip_encode = array('field_allowed_file_extensions')) {
			...
			foreach ($field as $key => $value)
				if (!is_array($value) && !in_array($key, $skip_encode))
					$field[$key] = ccfpgp_utils::encodeOption($value);
		...
		function insertFieldOption($option) {
			...
			foreach ($option as $key => $value)
				$option[$key] = ccfpgp_utils::encodeOption($value);
		...
		function insertStyle($style) {
			...
			foreach ($style as $key => $value)
					$style[$key] = ccfpgp_utils::encodeOption($value);
		...
		function updateForm($form, $fid, $skip_encode = array('form_access', 'form_fields','behavior')) {
			global $wpdb;
		...
		function insertUserData($data_object) {
			global $wpdb;
			//pgpchk
			$wpdb->insert(CCF_USER_DATA_TABLE, array('data_time' => $data_object->getDataTime(), 'data_formid' => $data_object->getFormID(), 'data_formpage' => $data_object->getFormPage(), 'data_value' => $data_object->getEncodedData(), 'data_private' => $data_object->getEncryptedData()));
            //echo '<br />data<br />';
            //print_r(array('data_time' => $data_object->getDataTime(), 'data_formid' => $data_object->getFormID(), 'data_formpage' => $data_object->getFormPage(), 'data_value' => $data_object->getEncodedData(), 'data_private' => $data_object->getEncryptedData()));
			return $wpdb->insert_id;
		...
		function getRolesArray() {
			...
			$out[] = __('Non-Registered User', 'ccf-pgp');
			return $out;
		}
