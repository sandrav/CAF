...
/*
	CCF PGP Plugin
*/
if (!class_exists('ccfpgpActivateDB')) {
	class ccfpgpActivateDB extends ccfpgpDB {
		var $cache = array();	

		function ccfpgpActivateDB($run_activate = true) {
			if ($run_activate)
				$this->activateDB();
		}
		...
		function createTables() {
			...
			if(!$this->formsTableExists()) {
				...
				$sql7 = " CREATE TABLE `".CCF_USER_DATA_TABLE."` (
						...
						`data_private` LONGTEXT NOT NULL ,
						PRIMARY KEY ( `id` )
						) ENGINE = MYISAM AUTO_INCREMENT=1 ";
				$wpdb->query($sql7);
		...
		function updateTables() {
			...
			if (!$this->columnExists('form_success_title', CCF_FORMS_TABLE))
				$wpdb->query("ALTER TABLE `" . CCF_FORMS_TABLE . "` ADD `form_success_title` VARCHAR( 150 ) NOT NULL DEFAULT '".__('Form Success!', 'ccf-pgp')."'");
			...
			//pgpchk
			//
			if (!$this->columnExists('field_encrypted', CCF_FIELDS_TABLE))
				$wpdb->query("ALTER TABLE `" . CCF_FIELDS_TABLE . "` ADD `field_encrypted`  INT( 1 ) NOT NULL DEFAULT '0'");
			//
		...
		function insertFixedFields() {
			$captcha = array('field_slug' => 'captcha', 'field_label' => __('Type the numbers.', 'ccf-pgp'), 'field_type' => 'Text', 'field_value' => '', 'field_maxlength' => '100', 'user_field' => 0, 'field_instructions' => __('Type the numbers displayed in the image above.', 'ccf-pgp'));
			$ishuman = array('field_slug' => 'ishuman', 'field_label' => __('Check if you are human.', 'ccf-pgp'), 'field_type' => 'Checkbox', 'field_value' => '1', 'field_maxlength' => '0', 'user_field' => 0, 'field_instructions' => __('This helps us prevent spam.', 'ccf-pgp'));
			$fixedEmail = array('field_slug' => 'fixedEmail', 'field_required' => 1, 'field_label' => __('Your Email', 'ccf-pgp'), 'field_type' => 'Text', 'field_value' => '', 'field_maxlength' => '100', 'user_field' => 0, 'field_instructions' => __('Please enter your email address.', 'ccf-pgp'));
			$fixedWebsite = array('field_slug' => 'fixedWebsite', 'field_required' => 1, 'field_label' => __('Your Website', 'ccf-pgp'), 'field_type' => 'Text', 'field_value' => '', 'field_maxlength' => '200', 'user_field' => 0, 'field_instructions' => __('Please enter your website.', 'ccf-pgp'));
			$emailSubject = array('field_slug' => 'emailSubject', 'field_required' => 1, 'field_label' => __('Email Subject', 'ccf-pgp'), 'field_type' => 'Text', 'field_value' => '', 'field_maxlength' => '200', 'user_field' => 0, 'field_instructions' => __('Please enter a subject for the email.', 'ccf-pgp'));
			$reset = array('field_slug' => 'resetButton', 'field_type' => 'Reset', 'field_value' => __('Reset Form', 'ccf-pgp'), 'user_field' => 0);
			$states = array('field_slug' => 'usaStates', 'field_label' => __('Select a State', 'ccf-pgp'), 'field_type' => 'Dropdown', 'user_field' => 0);
			$countries = array('field_slug' => 'allCountries', 'field_label' => __('Select a Country', 'ccf-pgp'), 'field_type' => 'Dropdown', 'user_field' => 0);
