...
/*
	CCF PGP Plugin
*/
if (!class_exists('ccfpgpExport')) {
	class ccfpgpExport extends ccfpgpDB {
		...
		function ccfpgpExport($option_name) {
		...
		function exportSavedFormSubmissionsToCSV($form_id = NULL) {
			...
			$out = array();
			$col_header = array(__('Form Submitted', 'ccf-pgp'), __('Date', 'ccf-pgp'),  __('Form Location', 'ccf-pgp'), __('Form ID', 'ccf-pgp'));
			$extra_cols = array();
			$out[] = $col_header;
		...
		function exportToFile($export_content = NULL) {
			...
			if (($export_handle = @fopen($this->getExportsPath() . $export_file, 'w')) == false)
				return false;
			$comment = '## ' . __('CCF PGP Export File', 'ccf-pgp') . "\n";
			$comment .= '## '. __('It is recommended that you do not edit this file. The order of the', 'ccf-pgp') . "\n";
			$comment .= '## ' . __('queries is important if you intend to use this file through the CCF', 'ccf-pgp') . "\n";
			$comment .= '## ' . __('exporter. The query to update general settings MUST be the last query', 'ccf-pgp') . "\n";
			$comment .= '## ' . __('in this file.', 'ccf-pgp') . "\n\n";
		...
		function importFromFile($file, $settings = array('mode' => 'clear_import', 'import_general_settings' => false, 'import_forms' => true,'import_fields' => true, 'import_field_options' => true, 'import_styles' => true, 'import_saved_submissions' => false)) {
			...
			if (move_uploaded_file($file['tmp_name'], $path . $file_name)) {
				...
				ccfpgpActivateDB::insertFixedFields();
		...
		function parseMultiQuery($sql, $unescape_semicolons = true, $replace_table_prefix = true) {
			...
			foreach ($commands as $k => $v) {
				...
				if ($replace_table_prefix)
					$commands[$k] = preg_replace('/^([a-zA-Z0-9 \s]+?)`.+?ccfpgp_(.+?)`/is', '$1 `' . $prefix . 'ccfpgp_$2`', $commands[$k]);
			}
			return $commands;
		}
