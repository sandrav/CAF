...
// variables and constructor are declared at the end
	...
	*/
	function load_strings() {
		...
		$this->use_profile_admin = "<p class=\"s2_message\">" . __('Usted puede modificar su suscripcion desde su', 'subscribe2') . " <a href=\"" . get_option('siteurl') . "/wp-admin/users.php?page=s2_users\">" . __('perfil', 'subscribe2') . "</a>.</p>";
		if ( $this->s2_mu === true ) {
		...
		$this->use_profile_users = "<p class=\"s2_message\">" . __('Usted puede modificar su suscripcion desde su', 'subscribe2') . " <a href=\"" . get_option('siteurl') . "/wp-admin/profile.php?page=s2_users\">" . __('perfil', 'subscribe2') . "</a>.</p>";
		if ( $this->s2_mu === true ) {
			global $blog_id, $user_ID;
			if ( !is_blog_user($blog_id) ) {
				// if we are on multisite and the user is not a member of this blog change the link
		...
		$this->confirmation_sent = "<p class=\"s2_message\">" . __('¡Muchas gracias por suscribirte!', 'subscribe2') . "</p>";

		$this->already_subscribed = "<p class=\"s2_error\">" . __('El email ya se encuentra subscrito .', 'subscribe2') . "</p>";

		$this->not_subscribed = "<p class=\"s2_error\">" . __('Este email no se encuentra subscrito', 'subscribe2') . "</p>";

		$this->not_an_email = "<p class=\"s2_error\">" . __('Por favor ingrese un e-mail válido.', 'subscribe2') . "</p>";

		$this->barred_domain = "<p class=\"s2_error\">" . __('Lo lamento, el email de ese dominio se encuentra marcado como SPAM. Por favor utilice un email alternativo', 'subscribe2') . "</p>";

		$this->error = "<p class=\"s2_error\">" . __('Lo lamento, parece que hubo un error en el servidor. Por favor intente luego.', 'subscribe2') . "</p>";

		$this->no_page = "<p class=\"s2_error\">" . __('You must to create a WordPress page for this plugin to work correctly.', 'subscribe2') . "<p>";

		$this->mail_sent = "<p class=\"s2_message\">" . __('Mensaje enviado!', 'subscribe2') . "</p>";

		$this->mail_failed = "<p class=\"s2_error\">" . __('Fallo el mensaje! Chequee su configuracion y la configuracion de su proveedor.', 'subscribe2') . "</p>";

		// confirmation messages
		$this->no_such_email = "<p class=\"s2_error\">" . __('No such email address is registered.', 'subscribe2') . "</p>";

		$this->added = "<p class=\"s2_message\">" . __('Se ha suscrito correctamente!', 'subscribe2') . "</p>";

		$this->deleted = "<p class=\"s2_message\">" . __('Se ha desuscripto correctamente.', 'subscribe2') . "</p>";
	...
	*/
	function shortcode($atts) {
		...
		if ( $hide == 'subscribe' ) {
			...
			$this->input_form_action = "<input type=\"submit\" name=\"subscribe\" value=\"" . __('Suscribirse »', 'subscribe2') . "\" />";
		} else {
		...
		// build default form
		$this->form = "<form method=\"post\" action=\"" . $url . "\"><input type=\"hidden\" name=\"ip\" value=\"" . $_SERVER['REMOTE_ADDR'] . "\" /><p><label for=\"s2email\">" . __('E-mail:', 'subscribe2') . "</label><br /><input type=\"text\" name=\"email\" id=\"s2email\" value=\"" . __('', 'subscribe2') . "\" size=\"20\" onfocus=\"if (this.value == '" . __('', 'subscribe2') . "') {this.value = '';}\" onblur=\"if (this.value == '') {this.value = '" . __('', 'subscribe2') . "';}\" /></p><p>" . $this->input_form_action . "</p></form>\r\n";
